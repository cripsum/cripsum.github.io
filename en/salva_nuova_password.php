...
if ($_SERVER['REQUEST_METHOD'] !== 'POST') {
    $messaggio = 'Method not allowed.';
} elseif (auth_rate_limited($mysqli, $resetIdentifier, 'password_reset_submit_failed', 5, 30)) {
    $messaggio = 'Too many attempts. Request a new link or try again later.';
    ...
    $messaggio = auth_password_policy_message($passwordPolicyError, 'en');
    auth_record_rate_failure($mysqli, null, $resetIdentifier, 'password_reset_submit_failed');
} else {
    ...
    if (!$stmt) {
        $messaggio = 'The link could not be verified.';
    } else {
    ...
    if (!empty($row)) {
        ...
        if ($contextPolicyError !== null) {
            $messaggio = auth_password_policy_message($contextPolicyError, 'en');
