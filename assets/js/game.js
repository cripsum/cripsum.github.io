(() => {
    'use strict';
    if (window.__cripsumDuelV13) return;
    window.__cripsumDuelV13 = true;

    const page = document.body?.dataset.page || '';
    const state = { matchId: Number(document.body?.dataset.matchId || 0) || null, roomCode:null, inventory:[], selectedTeam:[], match:null, poll:null, lastActionId:0, resultShown:false, lastChatId:0 };
    ...
    function rankBadge(rank){if(!rank)return'';const range=rank.range?` · ${rank.range}`:'';return `<span class="game-rank-badge" data-rank="${esc(rank.key)}" title="${esc(rank.label+range)}">${esc(rank.label)}</span>`}
    ...
    async function loadRanking(){const wrap=$('#rankingList'); if(!wrap)return; try{const d=await api('/api/game/get_ranking.php',{},'GET'); const rows=d.ranking||[]; if(!rows.length){wrap.innerHTML='<p class="game-hint">Classifica vuota.</p>';return} wrap.innerHTML=rows.map((r,i)=>`<div class="game-rank-row"><strong>#${i+1}</strong><span class="game-rank-name">${esc(r.username)}</span><span class="game-rank-meta">${rankBadge(r.rank)} <b>${r.rating}</b><small>${esc(r.rank?.range||'')}</small></span></div>`).join('')}catch(e){wrap.innerHTML='<p class="game-hint">Classifica non caricata.</p>';}}
    ...
    function bindCommon(){ $$('[data-action="find-match"]').forEach(b=>b.addEventListener('click',()=>findMatch(b.dataset.mode||'casual'))); $('[data-action="create-private"]')?.addEventListener('click',createPrivate); $('[data-action="join-code"]')?.addEventListener('click',joinCode); $('[data-action="active-match"]')?.addEventListener('click',activeMatch); $('[data-action="load-ranking"]')?.addEventListener('click',loadRanking); $$('[data-action="forfeit"]').forEach(b=>b.addEventListener('click',forfeit)); }
    document.addEventListener('DOMContentLoaded',()=>{bindCommon(); if(page==='duel-lobby'){loadProfile();loadRanking();setInterval(loadRanking,30000)} if(page==='duel-arena'){if(!state.matchId){showToast('Match mancante');return} $('#cardSearch')?.addEventListener('input',renderInventory); $('[data-action="submit-team"]')?.addEventListener('click',submitTeam); $$('[data-battle-action]').forEach(b=>b.addEventListener('click',()=>submitBattle(b.dataset.battleAction))); $('#chatForm')?.addEventListener('submit',(e)=>{e.preventDefault();sendChat();}); startPolling();}});
})();
