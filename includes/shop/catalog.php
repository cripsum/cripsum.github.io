...
function shop_product_view(array $row, string $lang, array $categories = []): array
{
    ...
    return [
        ...
        'category' => $categoryId !== null && isset($categories[$categoryId]) ? $categories[$categoryId]['slug'] : '',
    ];
}

function shop_products(mysqli $mysqli, int $vetrinaId, string $lang, array $categories): array
{
    $rows = shop_fetch_all(
        $mysqli,
    ...
    return array_map(static fn(array $row): array => shop_product_view($row, $lang, $categories), $rows);
}
...
function shop_product_for_checkout(mysqli $mysqli, string $slug, string $lang): ?array
{
    ...
    return [
        'product' => shop_product_view($row, $lang, shop_categories($mysqli, $vetrina['tipo'], $lang)),
        'vetrina' => $vetrina,
    ];
}
...
function shop_latest_merch(mysqli $mysqli, string $lang, int $limit = 8): array
{
    ...
    foreach ($rows as $row) {
        ...
        $view['url'] = '/' . $lang . '/merch/' . rawurlencode((string)$row['vetrina_slug']) . '?p=' . rawurlencode($view['slug']);
        $products[] = $view;
