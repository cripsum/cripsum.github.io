    ...
    <script>
        ...
        async function loadPosts() {
            try {
                ...
                try {
                    const response = await fetch('../api/content/get_posts.php?type=shitpost&limit=all');
                    const payload = await response.json();

                    if (payload && payload.ok && Array.isArray(payload.posts)) {
                        data = payload.posts.map(normalizeShitpostPost);
                    }
                ...
                if (!Array.isArray(data)) {
                    const response = await fetch('../api/get_shitpost.php?limit=all');
        ...
        function normalizeShitpostPost(post) {
            return {
                ...post,
                id: Number(post.id),
                titolo: post.titolo || '',
                ...
                tipo_foto_shitpost: post.tipo_foto_shitpost || post.media_mime || 'image/jpeg',
                foto_shitpost: post.foto_shitpost || null,
        ...
        function renderShitpostMedia(post) {
            const title = escapeHtml(post.titolo || 'Shitpost');

            if (post.media_url) {
                if (Number(post.is_video) === 1 || String(post.tipo_foto_shitpost || '').startsWith('video/')) {
                    return `
                ...
                return `
                    <div class="post-image">
                        <img src="${escapeHtml(post.media_url)}" alt="${title}">
                    </div>
                `;
            }
            ...
            return `
                <div class="post-image">
                    <img src="data:${escapeHtml(post.tipo_foto_shitpost || 'image/jpeg')};base64,${post.foto_shitpost || ''}" alt="${title}">
                </div>
            `;
        }
