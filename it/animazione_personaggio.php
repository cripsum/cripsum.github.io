...
if ($charData && !in_array($_SESSION['ruolo'] ?? 'utente', ['admin', 'owner'], true)) {
    ...
    if (!$ownsIt) {
        $charData = null;
        $errorMsg = 'Non possiedi ancora questo personaggio.';
    }
}
...
<body class="anim-page">
    ...
    <?php if (!empty($errorMsg)): ?>
        <!-- ══ ERRORE ══════════════════════════════════════════════════════════ -->
        <div id="anim-error">
            <h2><i class="fa-solid fa-circle-exclamation"></i> Personaggio non trovato</h2>
            <p><?= htmlspecialchars($errorMsg) ?></p>
            <a href="javascript:history.back()"><i class="fa-solid fa-arrow-left"></i> Torna indietro</a>
        </div>

    <?php else: ?>
        <!-- ══ BADGE ANTEPRIMA ═════════════════════════════════════════════════ -->
        <div id="anim-preview-badge"><i class="fa-solid fa-eye"></i> Anteprima animazione</div>
            ...
            <div class="gacha-phase gacha-phase--card" id="phase-card" style="display:none">
                ...
                <div id="anim-actions">
                    <a href="?id_personaggio=<?= $idPersonaggio ?>" class="gacha-btn gacha-btn--primary">
                        <i class="fa-solid fa-rotate-right"></i> Ripeti animazione
                    </a>
                    <a href="inventario" class="gacha-btn gacha-btn--ghost">
                        <i class="fa-solid fa-layer-group"></i> Inventario
                    </a>
