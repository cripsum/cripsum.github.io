    ...
    <script>
        ...
        async function loadPosts() {
            try {
                ...
                try {
                    const response = await fetch('../api/content/get_posts.php?type=rimasto&limit=all');
                    const payload = await response.json();

                    if (payload && payload.ok && Array.isArray(payload.posts)) {
                        data = payload.posts.map(normalizeRimastoPost);
                    }
                } catch (contentApiError) {
                    console.warn('Content API non disponibile, uso API vecchia:', contentApiError);
                }

                if (!Array.isArray(data)) {
                    const response = await fetch('../api/get_toprimasti.php?limit=all');
                    data = await response.json();
                }

                if (Array.isArray(data)) {
                    allPosts = data.sort((a, b) => parseInt(b.reazioni || 0) - parseInt(a.reazioni || 0));
        ...
        function normalizeRimastoPost(post) {
            return {
                ...post,
                id: Number(post.id),
                titolo: post.titolo || '',
                descrizione: post.descrizione || '',
                motivazione: post.motivazione || post.extra_text || '',
                username: post.username || 'Utente anonimo',
                data_creazione: post.data_creazione || post.created_at || '',
                tipo_foto_rimasto: post.tipo_foto_rimasto || post.media_mime || 'image/jpeg',
                foto_rimasto: post.foto_rimasto || null,
                media_url: post.media_url || null,
                is_video: Number(post.is_video || 0),
                reazioni: Number(post.reazioni ?? post.score ?? 0)
            };
        }
        ...
        function renderRimastoMedia(post, className = 'post-image-container', withOverlay = true) {
            const title = escapeHtml(post.titolo || 'Rimasto');
            const open = className === 'post-image-container' ? ` onclick="openPostModal(${post.id}, false)"` : '';

            let media = '';

            if (post.media_url) {
                if (Number(post.is_video) === 1 || String(post.tipo_foto_rimasto || '').startsWith('video/')) {
                    media = `<video src="${escapeHtml(post.media_url)}" class="post-image" controls preload="metadata"></video>`;
                } else {
                    media = `<img src="${escapeHtml(post.media_url)}" alt="${title}" class="post-image">`;
                }
            } else {
                media = `<img src="data:${escapeHtml(post.tipo_foto_rimasto || 'image/jpeg')};base64,${post.foto_rimasto || ''}" alt="${title}" class="post-image">`;
            }
            ...
            return `<div class="${className}"${open}>${media}${overlay}</div>`;
        }
        ...
        function openPostModal(postId, isPending) {
            ...
            modalContent.innerHTML = `
                <div class="modal-post">
                    ${renderRimastoMedia(post, 'modal-image', false)}
