(()=>{
    ...
    const t = {
        it: {
            ...
            status_error:   "Errore",
            img_alt:        "Immagine generata da GoonLand",
            err_load_img:   "Errore nel caricamento immagine",
            err_no_url:     "URL immagine mancante",
            err_proxy:      (status) => `Proxy immagini HTTP ${status}`,
            toast_generated:"Immagine generata",
            toast_err_load: "Errore nel caricamento. Riprova.",
            toast_no_img:   "Genera prima un'immagine",
            toast_download: "Download avviato",
            toast_dl_fail:  "Download non riuscito",
        },
        en: {
            ...
            status_error:   "Error",
            img_alt:        "Image generated by GoonLand",
            err_load_img:   "Image failed to load",
            err_no_url:     "Image URL missing",
            err_proxy:      (status) => `Image proxy HTTP ${status}`,
            toast_generated:"Image generated",
            toast_err_load: "Loading error. Try again.",
            toast_no_img:   "Generate an image first",
            toast_download: "Download started",
            toast_dl_fail:  "Download failed",
        },
    }[lang];

    if(window.__goonlandV2Loaded) return;    window.__goonlandV2Loaded = true;
    ...
    function startCooldown(){
        ...
        const tick = () => {
            if(countdown) countdown.textContent = t.wait_seconds(timeLeft);
            if(btn) btn.innerHTML = t.wait_btn(timeLeft);
    ...
    async function resolveImageUrl(contentType){
        ...
        if (!response.ok) {
            throw new Error(data?.error || t.err_proxy(response.status));
        }

        if (!data || typeof data.url !== "string" || !data.url.trim()) {
            throw new Error(t.err_no_url);
        }
    ...
    window.generateImage = async function(){
        ...
        try {
            ...
            await new Promise((resolve, reject) => {
                ...
                preload.onerror = () => reject(new Error(t.err_load_img));
