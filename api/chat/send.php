...
if ($isGif) {
    if (!chat_column_exists($mysqli, 'messages', 'message_type') || !chat_column_exists($mysqli, 'messages', 'media_url')) {
        chat_json(['ok' => false, 'error' => 'GIF non attive. Esegui lo SQL V2.1.'], 500);
    }

    if (mb_strlen($message, 'UTF-8') > MAX_MESSAGE_LENGTH) {
        chat_json(['ok' => false, 'error' => 'Caption troppo lunga.'], 422);
    }
...
if ($isGif) {
    if ($replyTo !== null) {
        ...
        if (!$stmt) chat_json(['ok' => false, 'error' => 'Errore server.'], 500);
        $stmt->bind_param('isssssis', $userId, $message, $type, $mediaUrl, $mediaPreviewUrl, $mediaTitle, $replyTo, $clientNonce);
    } else {
        $stmt = $mysqli->prepare('INSERT INTO messages (user_id, message, message_type, media_url, media_preview_url, media_title, created_at, client_nonce) VALUES (?, ?, ?, ?, ?, ?, NOW(), ?)');
        if (!$stmt) chat_json(['ok' => false, 'error' => 'Errore server.'], 500);
        $stmt->bind_param('issssss', $userId, $message, $type, $mediaUrl, $mediaPreviewUrl, $mediaTitle, $clientNonce);
    }
} else {
    if (chat_column_exists($mysqli, 'messages', 'message_type')) {
        $messageType = 'text';
        if ($replyTo !== null) {
            $stmt = $mysqli->prepare('INSERT INTO messages (user_id, message, message_type, reply_to, created_at, client_nonce) VALUES (?, ?, ?, ?, NOW(), ?)');
            if (!$stmt) chat_json(['ok' => false, 'error' => 'Errore server.'], 500);
            $stmt->bind_param('issis', $userId, $message, $messageType, $replyTo, $clientNonce);
        } else {
            $stmt = $mysqli->prepare('INSERT INTO messages (user_id, message, message_type, created_at, client_nonce) VALUES (?, ?, ?, NOW(), ?)');
            if (!$stmt) chat_json(['ok' => false, 'error' => 'Errore server.'], 500);
            $stmt->bind_param('isss', $userId, $message, $messageType, $clientNonce);
        }
    } else {
        if ($replyTo !== null) {
