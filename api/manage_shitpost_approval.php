...
try {
    $stmt = $mysqli->prepare("SELECT id, approvato FROM shitposts WHERE id = ?");
    ...
    $stmt = $mysqli->prepare("UPDATE shitposts SET approvato = ? WHERE id = ?");
    $stmt->bind_param("ii", $approved, $post_id);
    
    if ($stmt->execute()) {
        echo json_encode([
            'success' => true, 
            'message' => $approved == 1 ? 'Shitpost approvato con successo' : 'Shitpost disapprovato con successo'
        ]);
    } else {
