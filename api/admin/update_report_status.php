...
try {
    ...
    if (!in_array($source, ['content', 'chat', 'profile'], true)) admin_fail('Tipo segnalazione non valido.');
    ...
    if ($source === 'content') {
        ...
        if (!admin_table_exists($mysqli, 'profile_reports')) admin_fail('Tabella profile_reports mancante.', 500);

        $sets = ['status = ?'];
        $types = 's';
        $params = [$status];

        if (admin_column_exists($mysqli, 'profile_reports', 'reviewed_at')) {
            $sets[] = 'reviewed_at = ' . ($status === 'open' ? 'NULL' : 'NOW()');
        }
        if (admin_column_exists($mysqli, 'profile_reports', 'reviewed_by')) {
            $sets[] = 'reviewed_by = ?';
            $types .= 'i';
            $params[] = $status === 'open' ? null : (int)$adminUser['id'];
        }
        ...
        $stmt = $mysqli->prepare('UPDATE profile_reports SET ' . implode(', ', $sets) . ' WHERE id = ? LIMIT 1');
        if (!$stmt) admin_fail(admin_prepare_error($mysqli, 'Query aggiornamento segnalazione profilo non valida.'), 500);
        $stmt->bind_param($types, ...$params);
    } else {
