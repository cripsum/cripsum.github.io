...
try {
    ...
    $stmt->bind_param("i", $post_id);
    $stmt->execute();
    $deleted_votes = $stmt->affected_rows;

    $stmt = $mysqli->prepare("DELETE FROM toprimasti WHERE id = ?");
    $stmt->bind_param("i", $post_id);
    ...
    if ($stmt->execute()) {
        ...
        $mysqli->rollback();
        echo json_encode(['success' => false, 'error' => 'Errore nell\'eliminazione del post']);
    }
